...
local function move_plugin(plugin, moves, fs_state)
  ...
  if not success then
    ...
    moves[plugin.name] = { from = from, to = to, result = result.err(success) }
  else
    log.debug(fmt('Moved %s from %s to %s', plugin.name, from, to))
    moves[plugin.name] = { from = from, to = to, result = result.ok(success) }
  end
end
...
function M.update(plugins, update_plugins, disp, updates, opts)
  local tasks = {}
...
---@param moves {[string]: Result}
function M.fix_plugin_types(plugins, plugin_names, moves, fs_state)
